...
class HorizonFastDemo extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'dispatch:multi-process';
    ...
    public function handle()
    {
        ...
        $this->info('Dispatching jobs to multi process queue...');

        $jobIds = [];
        for ($i = 0; $i < 100; $i++) {
            $jobId = uniqid();
            $jobIds[] = $jobId;
            TimeConsumingJob::dispatch($jobId)->onQueue('multi_process');
            $this->info("Dispatched job {$jobId} to multi process queue.");
